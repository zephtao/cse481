...
class Nav2Markers(Node):
    def __init__(self):
      super().__init__('nav2marker')

      # subscribe to the initial pose topic
      self.init_pose_sub = self.create_subscription(PoseWithCovarianceStamped, 'initialpose', self.define_init_pose, 10)
      self.init_pose = Pose()
      ...
      with open('/home/hello-robot/cse481/zephyr_ws/goal_by_marker_pose.csv', mode='r') as goal_f:
        csvreader = csv.reader(goal_f, delimiter = ',')
      
        for pose_data in csvreader:
          #assuming csv data stored in this format
          self.goal.pose.position = Point(x=float(pose_data[0]), y=float(pose_data[1]), z=float(pose_data[2]))
          self.goal.pose.orientation = Quaternion(x=float(pose_data[3]), y=float(pose_data[4]), z=float(pose_data[5]), w=float(pose_data[6]))
    ...
    def nav_to_goal(self):
      ...
      self.get_logger().info(f'Navigating to goal: {self.goal}')
